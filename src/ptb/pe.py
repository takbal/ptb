...
def start(
    ...
    cp_path: str = "/data/scratch/grid/"
    + os.getenv("USER", default="UNKNOWN")
    + "/checkpoints/",
    priority: int = None,
    timeout: float = 45,
    ...
    cp_fname_bases = [
        os.path.join(cp_path, "%s__%s" % (idstr, jobids[i])) for i in range(len(inputs))
    ]

    log_fname_bases = [
        os.path.join(outdir, "%s" % jobids[i]) for i in range(len(inputs))
    ]
        ...
        while any(status < 4):
            ...
            if not all(status > 0):
                ...
                if debug:
                    ...
                    rets[nextidx] = func(
                        None, *inputs[nextidx][0], **inputs[nextidx][1]
                    )
                ...
                if not job.process.is_alive():
                    ...
                    if not job.pipe.poll():
                        ...
                        try:
                            ...
                            _log.error(
                                "error while reading pipe of %s" % jobids[job.idx]
                            )
                ...
                if rf.done():
                    ...
                    if rf.cancelled():
                        ...
                        try:
                            ...
                            _log.error(
                                "error while reading remote result of %s"
                                % jobids[jobidx]
                            )
                            rets[jobidx] = ParexecErrorReturn(traceback.format_exc())

                        if (
                            isinstance(rets[jobidx], ParexecErrorReturn)
                            and rets[jobidx].msg == "checkpointed"
                        ):
                            ...
                            rets[jobidx] = None
                            _log.info(
                                "%s is checkpointed, waiting for checkpoint file to appear"
                                % jobids[jobidx]
                            )
                        else:
                            status[jobidx] = 4
            ...
            for jobidx in range(num_jobs):
                ...
                if status[jobidx] == 6 and os.path.exists(
                    cp_fname_bases[jobidx] + ".checkpoint"
                ):
                    status[jobidx] = 0
                    _log.info(
                        "%s checkpoint file is available, re-scheduling"
                        % jobids[jobidx]
                    )
            ...
            if show_bars:
                ...
                bar_jobs.set_description_str(
                    "wait:%d,loc:%d,rem:%d,end:%d,err:%d,cpt:%d"
                    % (
                        st.count(0),
                ...
                running_jobs_indices = [
                    idx for idx, x in enumerate(st) if x == 1 or x == 2 or x == 3
                ]
...
def _pe_wrapper(
    func: Callable, args: tuple, pipe, timeout, cp_fname_base, log_fname_base
):
    """
    ...
    with open(log_fname_base + ".stdout", "a") as stdout, open(
        log_fname_base + ".stderr", "a"
    ) as stderr:
...
def _test_tqdm(num_bars):
    pbars = [
        tqdm.tqdm(
            total=100,
            desc=ptb.tools.rand_tag(int(np.ceil(10 * np.random.rand()))),
            bar_format="{percentage:3.0f}%|{bar} {desc} | {elapsed}->{remaining}",
        )
    ...
    for i in range(100):
        ...
        for idx in range(num_bars):
            ...
            pbars[idx].set_description_str(
                ptb.tools.rand_tag(int(np.ceil(10 * np.random.rand())))
            )
